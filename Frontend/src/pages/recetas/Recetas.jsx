...
const RECETA_DEMO = {
  ...
  grasas: 18,
  descripcion:
    "Un bowl nutritivo y equilibrado que combina la proteína magra del pollo con las grasas saludables del aguacate y los carbohidratos complejos del arroz integral. Perfecto para una comida completa.",
  ingredientes: [
  ...
  instrucciones: [
    {
      paso: 1,
      titulo: "Preparar el arroz",
      desc: "Cocina el arroz integral según las instrucciones del paquete. Normalmente tarda unos 20-25 minutos a fuego lento.",
    },
    {
      paso: 2,
      titulo: "Cocinar el pollo",
      desc: "Corta la pechuga de pollo en cubos. Salpimienta y saltéala con aceite de oliva en una sartén a fuego medio-alto durante 8-10 minutos.",
    },
    {
      paso: 3,
      titulo: "Preparar los vegetales",
      desc: "Lava las espinacas y corta los tomates cherry. Corta el aguacate en láminas finas.",
    },
    {
      paso: 4,
      titulo: "Armar el bowl",
      desc: "Coloca el arroz como base, añade el pollo, las espinacas, los tomates y las láminas de aguacate. Añade sal y pimienta al gusto.",
    },
  ],
};
    ...
    return (
      <div className="rec-panel rec-panel-despensa">
        ...
        <div className="rec-empty-despensa">
          <span>🔐</span>
          <p>Inicia sesión para ver tus ingredientes</p>
        </div>
      </div>
    );
  }
  ...
  if (cargando) {
    return (
      <div className="rec-panel rec-panel-despensa">
        ...
        <p className="rec-panel-sub">Cargando ingredientes...</p>
        <div className="rec-empty-despensa">
          <span>⏳</span>
          ...
          <div key={cat} className="rec-despensa-group">
            ...
            {items.map((p) => {
              ...
              return (
                <label
                  key={p.id}
                  className={`rec-despensa-item${sel ? " selected" : ""}`}
                >
                  <input
                    type="checkbox"
                    checked={sel}
                  ...
                  <span className="rec-despensa-qty">
                    {p.cantidad}
                    {p.unidad}
                  </span>
                </label>
        ...
        <div className="rec-result-overlay">
          ...
          <div className="rec-result-title-row">
            ...
            <button
              ...
              onClick={onGuardar}
              title={
                guardada ? "Guardado en biblioteca" : "Guardar en biblioteca"
              }
            >
              {guardada ? (
                <svg
          ...
          <div className="rec-vc-section">
            ...
            <div className="rec-vc-ingredientes">
              {receta.ingredientes.map((ing, idx) => (
                <label
                  key={idx}
                  className={`rec-vc-ing-item${ingredientesMarcados.includes(idx) ? " checked" : ""}`}
                >
                  <input
                    type="checkbox"
...
export default function Recetas() {
  ...
  // Cargar productos desde Firestore cuando el usuario está autenticado
  useEffect(() => {
    if (!user) {
      setProductos([]);
    ...
    async function cargarProductos() {
      try {
        const data = await getProductosFromStore();
        setProductos(data);
  ...
  const handleGenerar = async () => {
    if (!user) {
      alert("Debes iniciar sesión para generar recetas");
      return;
    }
    ...
    try {
      // Mapear ids a nombres de ingredientes
      const ingredientesNombres = ingredientesSeleccionados
        .map((id) => productos.find((p) => p.id === id)?.nombre)
        .filter(Boolean)
        .join(", ");

      if (!ingredientesNombres) {
      ...
      const payload = {
        ingredientes: ingredientesNombres,
        objetivo,
        tiempo: `${tiempo} min`,
        bajo_calorias: bajoCal,
        solo_una_olla: unaOlla,
        uid: user.uid,
      };
      ...
      const recetaFormateada = {
        ...
        match: Math.floor(Math.random() * 20) + 80, // match aleatorio (80-100%)
        tiempo: receta.tiempo_estimado,
        dificultad: receta.dificultad,
        calorias: receta.calorias,
        proteina: receta.proteina || 0,
        carbos: receta.carbos || 0,
        ...
        ingredientes: Array.isArray(receta.ingredientes)
          ? receta.ingredientes.map((ing) =>
              typeof ing === "string"
                ? { nombre: ing, detalle: "" }
                : { nombre: ing.nombre, detalle: ing.cantidad || "" },
            )
          : [],
  ...
  const handleGuardar = async () => {
    if (!user || !recetaGenerada) {
      alert("Error: usuario o receta no disponibles");
      return;
    }
        ...
        <div className="rec-layout">
          ...
          <div className="rec-panel-center">
            ...
            <button
              ...
              title={
                !user
                  ? "Inicia sesión para generar recetas"
                  : cargandoProductos
          ...
          <div className="rec-panel rec-panel-result">
            {!recetaGenerada && !generando && (
              <div className="rec-result-placeholder">
                ...
                <p>
                  Selecciona ingredientes y haz clic en{" "}
                  <strong>Generar Receta con IA</strong>
                </p>
              </div>
            )}
